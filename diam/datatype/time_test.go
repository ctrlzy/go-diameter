...
func TestTime(t *testing.T) {
	n := datatype.Time(time.Unix(1377093974, 0))
	...
	if n.Type() != datatype.TimeType {
		t.Fatalf("Unexpected type. Want %d, have %d",
			datatype.TimeType, n.Type())
	}
	if len(n.String()) == 0 {
...
func TestDecodeTime(t *testing.T) {
	b := []byte{0xd5, 0xbf, 0x47, 0xd6}
	v, err := datatype.DecodeTime(b)
	if err != nil {
		t.Fatal(err)
	}
	if n := time.Time(v.(datatype.Time)).Unix(); n != 1377093974 {
		t.Fatalf("Unexpected value. Want 1377093974, have %d", n)
	}
}

func BenchmarkTime(b *testing.B) {
	v := datatype.Time(time.Unix(1377093974, 0))
	for n := 0; n < b.N; n++ {
		v.Serialize()
	}
}
