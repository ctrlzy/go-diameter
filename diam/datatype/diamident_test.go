...
func TestDiameterIdentity(t *testing.T) {
	s := datatype.DiameterIdentity("hello")
	...
	if s.Type() != datatype.DiameterIdentityType {
		t.Fatalf("Unexpected type. Want %d, have %d",
			datatype.DiameterIdentityType, s.Type())
	}
	if len(s.String()) == 0 {
...
func TestDecodeDiameterIdentity(t *testing.T) {
	...
	s, err := datatype.DecodeDiameterIdentity(b)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal([]byte(s.(datatype.DiameterIdentity)), b) {
	...
	if v := string(s.(datatype.DiameterIdentity)); v != "hello, world" {
