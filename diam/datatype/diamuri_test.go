...
func TestDiameterURI(t *testing.T) {
	...
	if s.Type() != datatype.DiameterURIType {
		t.Fatalf("Unexpected type. Want %d, have %d",
			datatype.DiameterURIType, s.Type())
	}
	if len(s.String()) == 0 {
...
func TestDecodeDiameterURI(t *testing.T) {
	...
	s, err := datatype.DecodeDiameterURI(b)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal([]byte(s.(datatype.DiameterURI)), b) {
	...
	if v := string(s.(datatype.DiameterURI)); v != "hello, world" {
