...
func TestIPv6(t *testing.T) {
	ip6 := datatype.IPv6(net.ParseIP("2001:0db8::ff00:0042:8329"))
	...
	if ip6.Type() != datatype.IPv6Type {
		t.Fatalf("Unexpected type. Want %d, have %d",
			datatype.IPv6Type, ip6.Type())
	}
	if len(ip6.String()) == 0 {
...
func TestIPv6Malformed(t *testing.T) {
	ip6 := datatype.IPv6(net.ParseIP("10.0.0.1"))
...
func TestDecodeIPv6(t *testing.T) {
	b := []byte{0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x42, 0x83, 0x29}
	ip6, err := datatype.DecodeIPv6(b)
	if err != nil {
		t.Fatal(err)
	}
	if ip := net.IP(ip6.(datatype.IPv6)).String(); ip != "2001:db8::ff00:42:8329" {
