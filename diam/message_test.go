...
func TestReadMessage2(t *testing.T) {
	t.Logf("try to read OFR len %d", len(testMessage2))
	msg, err := diam.ReadMessage(bytes.NewReader(testMessage2), dict.Default)
	if err != nil {
		t.Fatal(err)
	}
	ofr := &smparser.OFR{}
...
func BenchmarkReadMessage2(b *testing.B) {
	reader := bytes.NewReader(testMessage2)
	for n := 0; n < b.N; n++ {
		diam.ReadMessage(reader, dict.Default)
		reader.Seek(0, 0)
	}
