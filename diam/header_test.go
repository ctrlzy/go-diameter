...
func TestDecodeHeader(t *testing.T) {
	hdr, err := diam.DecodeHeader(testHeader)
	if err != nil {
		t.Fatal(err)
	}
	switch {
	...
	case hdr.CommandFlags != diam.RequestFlag:
		t.Fatalf("Unexpected CommandFlags. Want %#x, have %#x", diam.RequestFlag, hdr.CommandFlags)
...
func TestDecodeHeaderMalformed(t *testing.T) {
	_, err := diam.DecodeHeader(testHeader[:10])
	if err == nil {
