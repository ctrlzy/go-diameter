...
func TestQoSFilterRule(t *testing.T) {
	s := datatype.QoSFilterRule("hello")
	...
	if s.Type() != datatype.QoSFilterRuleType {
		t.Fatalf("Unexpected type. Want %d, have %d",
			datatype.QoSFilterRuleType, s.Type())
	}
	if len(s.String()) == 0 {
...
func TestDecodeQoSFilterRule(t *testing.T) {
	...
	s, err := datatype.DecodeQoSFilterRule(b)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal([]byte(s.(datatype.QoSFilterRule)), b) {
	...
	if v := string(s.(datatype.QoSFilterRule)); v != "hello, world" {
